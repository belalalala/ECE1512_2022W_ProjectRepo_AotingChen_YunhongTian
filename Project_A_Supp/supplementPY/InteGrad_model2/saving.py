...
if TYPE_CHECKING:
    from interfaces import Explainer
    import AnchorImage
    import AnchorText
    import IntegratedGradients
    from shap_wrappers import KernelShap
    from shap_wrappers import TreeShap
...
def _load_AnchorText(path: Union[str, os.PathLike], predictor: Callable, meta: dict) -> 'AnchorText':
    ...
    if sampling_strategy in nlp_sampling:
        ...
        # load language model
        import lang_model as lang_model
...
def _load_CounterfactualRL(path: Union[str, os.PathLike],
    ...
    if explainer.params["backend"] == Framework.TENSORFLOW:
        import backends_tensorflow_cfrl_base as backend
    else:
        import backends_pytorch_cfrl_base as backend  # type: ignore
...
def _load_CounterfactualRLTabular(path: Union[str, os.PathLike],
    ...
    # select backend module
    if explainer.params["backend"] == Framework.TENSORFLOW:
        import backends_tensorflow_cfrl_tabular as backend
    else:
        import backends_pytorch_cfrl_tabular as backend  # type: ignore
